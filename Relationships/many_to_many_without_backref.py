...
db = SQLAlchemy('sqlite:///m2m_wb.db')
...
class Parent(db.Model):
    ...
    children = db.relationship("Child",secondary=association_table,lazy='dynamic')

class Child(db.Model):
    __tablename__ = 'right'
    id = db.Column(Integer, primary_key=True)
    guardians=db.relationship('Parent',secondary=association_table,lazy='dynamic')
